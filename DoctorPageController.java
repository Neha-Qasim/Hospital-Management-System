...
public class DoctorPageController implements Initializable {
    		...
    		try {
    			...
    			if(!result.next()) {
        			...
        			if(result.next()) {
        				...
        				Parent root = loader.load(); // This triggers the FXML loading and injection
        				DoctorMainFormController controller = loader.getController(); // Access controller if needed
    ...
    void registerAccount() {
    	if(register_fullName.getText().isEmpty()||register_email.getText().isEmpty()||register_doctorID.getText().isEmpty()||register_password.getText().isEmpty()) {
    		...
    		String checkDoctorID = "SELECT * FROM doctor WHERE doctor_id='" + register_doctorID.getText().trim() + "'";
    		...
    		try {
    			...
    			{
    				alert.errorMessage("Invaid password atleast 8 characters needed");	
    			}else {
